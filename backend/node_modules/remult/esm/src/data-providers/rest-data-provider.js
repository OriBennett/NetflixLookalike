        ...
        for (const key in options.include) {
            if (Object.prototype.hasOwnProperty.call(options.include, key)) {
                ...
                if (typeof element === 'object') {
                    ...
                    if (rel) {
                        element = findOptionsToJson(element, new Remult().repo(rel.toType()).metadata);
                    }
                }
                ...
                for (const key in newInclude) {
                    if (Object.prototype.hasOwnProperty.call(newInclude, key)) {
                        ...
                        if (typeof element === 'object') {
                            const rel = getRelationInfo(meta.fields.find(key).options);
                            if (rel) {
                                element = findOptionsFromJson(element, new Remult().repo(rel.toType()).metadata);
                            }
                        }
                        newInclude[key] = element;
                    }
        ...
        return {
            ...
            subscribe: async (queryId) => {
                ...
                return {
                    result,
                    unsubscribe: async () => {
                        return actionInfo.runActionWithoutBlockingUI(() => this.http().post(this.url() + '?__action=endLiveQuery', {
                            id: queryId,
                        }));
                    },
                };
