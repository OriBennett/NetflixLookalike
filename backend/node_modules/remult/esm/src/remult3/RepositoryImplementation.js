...
export class RepositoryImplementation {
    ...
    async createAfterFilter(orderBy, lastRow) {
        let values = new Map();
    ...
    relations(item) {
        return new Proxy({}, {
            get: (target, key) => {
                ...
                let repo = this.remult.repo(rel.toType());
                let { findOptions, returnNull, returnUndefined } = this.findOptionsBasedOnRelation(rel, field, undefined, item, repo);
                const toRepo = new RepositoryImplementation(repo.entity, repo.remult, repo.dataProvider, repo._info, findOptions);
                if (rel.type === 'toMany')
                    return toRepo;
                else
                    return {
    ...
    constructor(entity, remult, dataProvider, _info, defaultFindOptions) {
        this.entity = entity;
        this.remult = remult;
    ...
    idCache = new Map();
    getCachedById(id, doNotLoadIfNotFound) {
        id = id + '';
    ...
    async getCachedByIdAsync(id, doNotLoadIfNotFound) {
        id = id + '';
        let r = this.idCache.get(id);
        if (r instanceof Promise)
            return await r;
    ...
    async delete(item) {
        ...
        let ref2 = this.getRefForExistingRow(item, undefined);
        if (!this.dataProvider.isProxy)
            await ref2.reload();
        return ref2.delete();
    }
    ...
    getRefForExistingRow(entity, id) {
        let ref = getEntityRef(entity, false);
        if (!ref) {
    ...
    async rawFind(options, skipOrderByAndLimit = false, loader) {
        if (!options)
            options = {};
    ...
    async buildEntityDataProviderFindOptions(options) {
        let opt = {};
        opt = {};
    ...
    async fromJsonArray(jsonItems, loadOptions) {
        const loader = new RelationLoader();
        const result = await this.loadManyToOneForManyRows(jsonItems.map((row) => {
            let result = {};
    ...
    async loadManyToOneForManyRows(rawRows, loadOptions, loader) {
        ...
        for (const col of this.metadata.fields) {
            ...
            if (ei) {
                let isRelation = col.options?.[relationInfoMember];
                if (!isRelation) {
                    let load = !col.options.lazy;
                    if (loadFields !== undefined)
                        load = loadFields.includes(col);
                    if (load) {
                        let repo = this.remult.repo(col.valueType);
                        let toLoad = [];
                ...
                const rel = col.options[relationInfoMember];
        ...
        for (const col of this.metadata.fields) {
            ...
            if (rel && incl) {
                ...
                for (const row of result) {
                    let { findOptions, returnNull } = this.findOptionsBasedOnRelation(rel, col, incl, row, otherRepo);
                    if (returnNull)
                        row[col.key] = null;
                    else {
                        const entityType = rel.toType();
                        const toRepo = this.remult.repo(entityType);
                        loader
                            .load({
                            entityType,
    ...
    /*@internal */
    findOptionsBasedOnRelation(rel, field, moreFindOptions, row, otherRepo) {
        ...
        function buildError(what) {
            return Error(`Error for relation: "${field.key}" to "${otherRepo.metadata.key}": ` +
                what);
        }
        ...
        if (options.field) {
            ...
            let val = rel.type === 'reference'
                ? getEntityRef(row).fields.find(requireField(field.key, this.metadata)).getId()
                : row[field.key];
    ...
    async mapRawDataToResult(r, loadFields) {
        if (!r)
            return undefined;
    ...
    create(item) {
        ...
        if (this.defaultFindOptions?.where) {
            __updateEntityBasedOnWhere(this.metadata, this.defaultFindOptions.where, r);
            this.fixTypes(r);
        }
    ...
    /* @internal*/
    async translateWhereToFilter(where) {
        if (!where)
            where = {};
        if (this.defaultFindOptions?.where) {
            let z = where;
...
export const columnsOfType = new Map();
export function createOldEntity(entity, remult) {
    let r = columnsOfType.get(entity);
    if (!r)
...
class rowHelperBase {
    ...
    constructor(fieldsMetadata, instance, remult, isNewRow) {
        ...
        for (const col of fieldsMetadata) {
            let ei = getEntitySettings(col.valueType, false);
            if (ei && remult) {
                let lookup = new LookupColumn(remult.repo(col.valueType), Boolean(getRelationInfo(col.options)), col.allowNull);
...
export function getControllerRef(container, remultArg) {
    ...
    if (!result) {
        let columnSettings = columnsOfType.get(container.constructor);
        if (!columnSettings)
...
export function getValueList(type) {
    {
        const fr = type?.metadata?.valueType;
        if (fr)
            return ValueListInfo.get(fr).getValues();
    }
    {
        const fr = type?.valueType;
        if (fr)
            return ValueListInfo.get(fr).getValues();
    }
