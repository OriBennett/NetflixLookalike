...
class KnexEntityDataProvider {
    ...
    async init() {
        const r = (await dbNamesOf(this.entity));
        return {
            $dbNameOf: (f) => {
                let fm = f;
    ...
    async insert(data) {
        ...
        if (this.entity.idMetadata.field instanceof CompoundIdField)
            resultFilter = resultCompoundIdFilter(this.entity.idMetadata.field, undefined, data);
        else
...
class FilterConsumerBridgeToKnexRequest {
    ...
    isIn(col, val) {
        this.result.push((knex) => knex.whereIn(this.nameProvider.$dbNameOf(col), val.map((x) => translateValueAndHandleArrayAndHandleArray(col, x))));
    }
    isEqualTo(col, val) {
        this.add(col, val, '=');
    }
...
export class KnexSchemaBuilder {
    ...
    async createIfNotExist(entity) {
        const e = await dbNamesOf(entity);
